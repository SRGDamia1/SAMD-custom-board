...
All of the information needed for the pin descriptions to fill in the array for your new variant comes from the multiples tables from the SAMD datasheets.
...
## What you MUST get right in your variants file

It is absolutely crucial that you have correctly set:

- ***THE ORDER***
- The port letter
- The number within the port

These three things map the physical pin to your Arduino "digital pin number."
Don't get it wrong!

You also need to get channels right for analog, PWM, timer, and interrupt functionality.
To find the right channel for each port and pin within the port, look at the multiplexing table for your processor.
That applies to these fields:

- [EAnalogChannel](#analog-channel)
- [EPWMChannel](#pwm-channel)
- [ETCChannel](#tc-channel)
- [EExt_Interrupts](#external-interrupt-channel)

The other two fields, [PioType/pin type](#piotype) and [pin attribute](#pin-attributes) are badly documented and - in most cases - *it doesn't matter what you put in those fields.*
Most of the values in the pin type field are place holders - the core and libraries don't use them at all.
In some cases, they are intended to align with values that are used in some other processor type core, but still mean nothing here.
The only pins that are important to get right in the PioType field are the SERCOM pins.
The values in the pin attributes fields are only important for analog, PWM, and timer pins.
...
This is used as a guide for setting the pin control registries for each pin.
For USART or I2C/Wire functions to work, you ***must*** correctly specify the `PIO_SERCOM` functionality in the EPioType field for that pin.
Most of the other libraries will attempt to set the pin functionality as needed **regardless of what you put in this field**.
So, for the most part, except for pins that you want to always be preconfigured for users to SERCOM functionality, it doesn't really matter what you put here.
...
| EPioType         | Letter for Peripheral in Multiplexing Table | Applicable Processor | Peripheral Use / Description                                                                               | Notes                                                                                                                                                                                                                                                                                                                                                                                           |
| ---------------- | ------------------------------------------- | -------------------- | ---------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PIO_NOT_A_PIN    | -                                           | All SAMD             | Use this for dummy lines in your array.                                                                    | This is usually used to assign a pin for the USB Host enable on a board that has no such pin.                                                                                                                                                                                                                                                                                                   |
| PIO_EXTINT       | A                                           | All SAMD             | External Interrupt Control, the pins are used as interrupts                                                | The coding for pin interrupts in WInterrupts.c will force a pin to be controlled by the interrupt peripheral regardless of whether the pin is set as PIO_EXTINT in the pin description.  You must set the interrupt channel for the pin correctly in the EExt_Interrupts field.                                                                                                                 |
| PIO_ANALOG       | B                                           | All SAMD             | Analog-to-digital converter (ADC), digital-to-analog converter (DAC), or Peripheral Touch Controller (PTC) | You must set the analog channel for the pin correctly in the EAnalogChannel field.  You must also specify the first or second ADC in the pin attributes field.  The coding for analog functionality in wiring_analog will force a pin to be controlled by the analog peripheral when you attempt to use analog functionality regardless of whether the pin is set as PIO_ANALOG as the pin type |
| PIO_SERCOM       | C                                           | All SAMD             | SERCOM using primary pad configuration                                                                     | **If you do not set this, your pin will not be readily usable for USART or I2C/Wire.**  You cannot combine SERCOM and SERCOM_ALT pins in a single SERCOM configuration.                                                                                                                                                                                                                         |
| PIO_SERCOM_ALT   | D                                           | All SAMD             | SERCOM using alternate pad configuration                                                                   | **If you do not set this, your pin will not be readily usable for USART or I2C/Wire.**  You cannot combine SERCOM and SERCOM_ALT pins in a single SERCOM configuration.                                                                                                                                                                                                                         |
| PIO_TIMER        | E                                           | All SAMD             | Timer Capture Input                                                                                        | You must set the timer channel correctly in the EPWMChannel or ETCCChannel field. The peripheral used for the timer is determined by the pin attribute field.                                                                                                                                                                                                                                   |
| PIO_TIMER_ALT    | F                                           | All SAMD             | Timer Compare Output                                                                                       | You must set the timer channel correctly in the EPWMChannel or ETCCChannel field. The peripheral used for the timer is determined by the pin attribute field.                                                                                                                                                                                                                                   |
...
| PIO_DIGITAL      | No peripheral, general I/O                  | All SAMD             | Use these for pins that will be used for basic I/O and directly controlled by the I/O port registers       |                                                                                                                                                                                                                                                                                                                                                                                                 |
...
The pin attribute (`uint32_t ulPinAttribute`) field of the _PinDescription struct is to specify which ADC to use for analog readings or which of the multiple possible peripheral sets to use for PWM.
*Attributes not related to the ADC or PWM are not used by the SAMD core!*
In some cases, they are placeholders to align with values that are used by other cores.

For pins where you need to specify *both* analog and PWM peripherals, you can put a set of values here, not just a single attribute for each pin.
...
| -------------------------- | ------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PIN_ATTR_NONE              | (0UL<<0)      | All SAMD             | **Not used by SAMD core**; labeled "Pin has no extra use attriburtes" [sic]                                                                                                                                                                                                                                   |
| PIN_ATTR_COMBO             | (1UL<<0)      | All SAMD             | **Not used by SAMD core**                                                                                                                                                                                                                                                                                     |
| PIN_ATTR_EXTINT            | (1UL<<6)      | All SAMD             | **Not used by SAMD core** External interrupt channels are set by EExt_Interrupts field; WInterrupts.c only checks for a value in that field.                                                                                                                                                                  |
| PIN_ATTR_DIGITAL           | (1UL<<2)      | All SAMD             | **Not used by SAMD core**; most variants files use this as a placeholder value for pins without analog or PWM abilities.                                                                                                                                                                                      |
...
As mentioned in the pin attributes table, pins controlled by the PDEC peripheral that have a PDEC channel instead of a TCC channel do not support PWM.
...
If your pin cannot be used as an external interrupt (this should only be the NMI pin), the value should be set to `NOT_AN_INTERRUPT `.
